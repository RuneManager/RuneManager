            ...
            Task.Factory.StartNew(() =>
            {
                while (true)
                {
                    ...
                    if (freshMons.TryTake(out fresh))
                    {
                        ...
                        if (currentList.Count < buildsShow || fresh.score > currentList.Min(qq => qq.score))
                        {
                            currentList.Add(fresh); //.Where(IsOkayBuild)
                            currentList = currentList.OrderByDescending(m => m.score).Take(buildsShow).ToList();
                            hasChanged = true;
                        }
                    }
                    ...
                    if (changedSpec && monsList.Count > 0)
                    {
                        ...
                        currentList = monsList.Where(m => m.Value != null).Select(m => m.Value).OrderByDescending(m => m.score).Take(buildsShow).ToList();
        ...
        private void RegenList()
        {
            Task.Factory.StartNew(() =>
            {
                ...
                lock (listGenLock)
                {
                    ...
                    var llist = monsList.Where(m => m.Value != null).Select(m => m.Value).OrderByDescending(m => m.score).Take(buildsShow);
