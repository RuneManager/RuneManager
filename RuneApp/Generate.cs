    ...
    public partial class Generate : Form {
        ...
        private ListViewItem renderLoadoutTest(Monster m) {
            ...
            if (Program.Settings.TestGray && m.Current.Runes.Any(r => r.UsedInBuild))
                ...
                if (m.Current.Sets.Any(rs => RuneProperties.MagicalSets.Contains(rs) && rs.Size() == 2) &&
                    m.Current.Sets.Any(rs => RuneProperties.MagicalSets.Contains(rs) && rs.Size() == 4)) {
                    li.ForeColor = Color.Green;
                }
                else if (m.Current.Sets.Any(rs => RuneProperties.MagicalSets.Contains(rs) && rs.Size() == 2)) {
                    li.ForeColor = Color.Goldenrod;
                }
                else if (m.Current.Sets.Any(rs => RuneProperties.MagicalSets.Contains(rs) && rs.Size() == 4)) {
