...
namespace RuneApp.Controls {
    ...
    public class RichTextBoxEx : RichTextBox {
        ...
        public void AppendLink(string text) {
            InsertLink(text, this.Text.Length);
        }

        public void AppendLink(string text, string link) {
            InsertLink(text, link, this.Text.Length);
        }
        ...
        private void SendSelectionStyle(CHARFORMAT2_STRUCT cf, UInt32 mask, UInt32 effect) {
            cf.cbSize = (UInt32)Marshal.SizeOf(cf);
            cf.dwMask = mask;
            cf.dwEffects = effect;
