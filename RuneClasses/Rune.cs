...
namespace RuneOptim
{
	...
	public class Rune : RuneLink
	{
		...
		public int Rarity
		{
			...
			{
				return Subs.Count;
				/*if (Subs[0].Type == Attr.Null) return 0; // Normal
				if (Subs[1].Type == Attr.Null) return 1; // Magic
				if (Subs[2].Type == Attr.Null) return 2; // Rare
				if (Subs[3].Type == Attr.Null) return 3; // Hero
				return 4; // Legend*/
			}
		}
		...
		public Rune(Rune rhs)
		{
			...
			Swapped = rhs.Swapped;
			/*Main.Type = rhs.Main.Type;
			Main.Value = rhs.Main.Value;
			Innate.Type = rhs.Innate.Type;
			Innate.Value = rhs.Innate.Value;
			Subs[0].Type = rhs.Subs[0].Type;
			Subs[0].Value = rhs.Subs[0].Value;
			Subs[1].Type = rhs.Subs[1].Type;
			Subs[1].Value = rhs.Subs[1].Value;
			Subs[2].Type = rhs.Subs[2].Type;
			Subs[2].Value = rhs.Subs[2].Value;
			Subs[3].Type = rhs.Subs[3].Type;
			Subs[3].Value = rhs.Subs[3].Value;*/
		}
